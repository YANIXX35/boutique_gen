...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        switch ($_POST['action']) {
            ...
            case 'toggle_admin':
                $userId = $_POST['user_id'];
                $isAdmin = $_POST['is_admin'] == 1 ? 0 : 1;
                
                // Empêcher de retirer les droits admin à soi-même
                if ($userId == $_SESSION['user_id'] && $isAdmin == 0) {
                    echo json_encode(['success' => false, 'message' => 'Vous ne pouvez pas retirer vos propres droits admin']);
                    break;
                }
                ...
                $action = $isAdmin == 1 ? 'accordés' : 'retirés';
                echo json_encode(['success' => true, 'message' => "Droits admin $action avec succès"]);
                break;
